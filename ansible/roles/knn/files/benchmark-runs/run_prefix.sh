generate_prefix_args() {
     ...
     local arg_string="${EXECUTABLE_DIR}/${EXECUTABLE} -l ${fulllabel} -a ${ALGORITHM} -c ${concurrency} -f ${dataset_file} -k ${k} -m ${MODE} -s ${sample} -v ${VECTOR} -g ${GROUPING} -y ${MIN_BATCH_SIZE} -z ${MAX_BATCH_SIZE}"
     echo "${arg_string}"
}
...
if [[ ${PERFORM_STANDARD} == "true" ]]; then
  ...
  for dataset in "${DATASETS[@]}"; do
      for k in "${K_VALUES[@]}"; do
          arg_string=$(generate_prefix_args "${dataset}" "${k}" "${SAMPLE}" "${CONCURRENCY}")
          ARGS+=("${arg_string}")
      done
  done
...
if [[ ${PERFORM_SCALABILITY} == "true" ]]; then
  ...
  for concurrency in "${SCAL_CONCURRENCIES[@]}"; do
    ...
    for dataset in "${DATASETS[@]}"; do
      arg_string=$(generate_prefix_args "${dataset}" "${SCAL_K}" "${sample}" "${concurrency}")
