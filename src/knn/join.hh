...
public:
  ...
  nlohmann::json get_json_statistics() {
    nlohmann::json json;
    json["index"] = index.get_statistics().to_json();
  ...
  void do_multiple_lookups(Iterator begin, Iterator end, const int32_t concurrency) {
    ...
    arena.execute([&] {
      ...
      oneapi::tbb::parallel_for(iter, [&](const tbb::blocked_range<Iterator>& r) {
        ...
        state.overlaps.resize(Base::records.size());
  ...
  void lookup(const int32_t record_id, PosThreadState& state) {
    ...
    while (!q.empty()) {
      ...
      if constexpr (use_index_pos()) {
        ...
        if (p_ub == e.p_s) {
          index.read_cross(record, e.lengrp_ptr, e.p_r, e.p_s, candidates, overlaps);
        } else {
          // todo clean up
          if (q.probe_vertical(r_size, r_grp, e)) {
            index.read_vertical_filtered(record, e.lengrp_ptr, e.p_r, p_ub, candidates, overlaps);
          } else {
            auto token = record.tokens[e.p_r];
            index.read_horizontal_filtered(token, e.lengrp_ptr, e.p_r, p_ub, candidates, overlaps);
          }
        }
      } else {
...
private:
  ...
  std::vector<int32_t> falsification_idx;
